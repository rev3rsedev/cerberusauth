...
> Status: v0.1 (MVP). This document describes what exists today and records
> the reasoning behind the load-bearing decisions. When behavior and this
> document disagree, the document is wrong; fix it.
...
service and HTTP layers run against `storetest.FakeStore`, so `make test`
needs no database.
...
contributor must install and keep in sync, for marginal benefit at this
size. Hand-written pgx queries live in one file (`internal/store/postgres`),
typed against the `store.Store` interface, so the blast radius of a bad
query is contained and the swap to sqlc later is mechanical if the query
count grows.

### stdlib `net/http` instead of chi
Go 1.22+ pattern routing (`POST /v1/client/validate`, `{id}` path values)
covers every route we have. Middleware is a handful of small functions
(recover, logging, admin auth, login rate limit). chi earns its keep on
bigger route trees; here it would be a dependency that saves nothing.
...
also needs the master key from the environment. Limitation: an attacker who
...
payload, so a client that "forgets" to verify has nothing convenient to
read.
...
current `server_ts`, so the attacker gains nothing they couldn't get by
...
| Data | Storage | Why |
|---|---|---|
| License keys | SHA-256 of canonical form | DB dump doesn't leak usable keys. High entropy (125 bits) means a plain hash is fine, no salt needed. Plaintext shown exactly once, at issuance. |
| Admin passwords | argon2id (19 MiB, t=2, p=1, the OWASP baseline), PHC string | Standard password storage. |
...
the response time doesn't reveal account existence. Login is also
rate-limited per IP (token bucket: burst 5, one attempt refilled every 10s;
see `internal/server/ratelimit.go`). The limiter keys on the TCP peer
address, deliberately not `X-Forwarded-For`, so behind a reverse proxy it
sees only the proxy's IP; rate-limit at the proxy in that topology.
...
```
POST   /v1/admin/login
DELETE /v1/admin/token                    revoke the presented token (logout)
...
Does **not** protect against: a patched client binary (no server-side check
survives one; see the README threat model), a compromised server or leaked
master key, key sharing before first HWID bind, denial of service by volume
(only login has a limiter in v0.1; front with a proxy).
